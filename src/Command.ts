...
function getParensAtIndex(signals: number[], index: number): [number[], number] {
...
function getNumberedObjectAtIndex(signals: number[], index: number): [number[], number] {
...
function getValueAtIndex(signals: number[], index: number): [number[] | null, number] {
    if (index === -1) {
        return [null, -1];
...
function getSentenceFromPrepositions(prepositions: Preposition[], sender: number): Map<number | null, number[]> {
...
function editSignalsInChannel(signals: number[], edit: (signals: number[]) => number[]): number[] {
    if (signals[0] === -65535) {
...
function substringMatches(signals: number[], index: number, find: number[]): boolean {
...
function substitute(signals: number[], find: number[], replace: number[]): number[] {
    const newSignals = [];
...
function applyCommandToMessage(target: Message, command: Action) {
    switch (command.type) {
        case "break": return breakMessage(target, command);
        case "replace": return replaceMessage(target, command);
        case "substitute": return substituteInMessage(target, command);
    }
}

function findTarget(history: Message[], sender: number, channel: number | null, target?: number): Message {
    for (let i = history.length - 2; i >= 0; i--) {
        const message = history[i];
        // Correct sender and channel
        if (message.sender === sender && getChannel(message.signals) === channel
...
function applyCommandToHistory(history: Message[], command: Command) {
    const target = findTarget(history, command.sender, command.channel, command.target);
