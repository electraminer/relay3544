...
const COLORS = [
  0xff5800, 0xbbff00, 0x00cdff, 0x0084ff, 0x4d00ff, 0xfb39ff, 0xff0fd7,
  0x484848, 0x636363, 0xffffff, 0xffffff,
];
    ...
    return (
      <Table
        ...
        onChangeCell={(row, col, value) =>
          changeImage(updateCell(row, col, value))
        }
        onDeleteRow={(row) =>
          changeImage((prev) => prev.filter((_, i) => i !== row))
        }
        onAddRow={() =>
          changeImage((prev) => [...prev, ["0", "0", "0", "0", "0"]])
        }
        cellStyle={(_row, col, value) => {
          if (col !== 4) return undefined;
          try {
            return {
              backgroundColor: `#${lerpColor(getColor(parseInt(value)), 0, 0.5).toString(16)}`,
            };
          } catch (e) {
