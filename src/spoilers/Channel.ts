...
export function sentInsideChannel(signals: number[], channel: number | null): number[] {
    if (channel === null || channel === -65536) return signals;
    return [-65535, channel, ...signals];
}

export function displayedInsideChannel(signals: number[], channel: number | null): number[] {
    if (channel === null || channel === -65536) return signals;
