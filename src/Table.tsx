...
export function Table(props: {
  ...
  cellStyle?: (row: number, col: number, value: string) => CSSProperties | undefined,
  onFocusCell?: (row: number, col: number) => void,
  onFocusDeleteRow?: () => void,
  ref?: React.Ref<HTMLDivElement>,
}) {
  const {
    columns, dataColumnWidths, rows, onChangeCell, onDeleteRow, onAddRow,
    cellClassName, cellStyle, onFocusCell, onFocusDeleteRow, ref,
  } = props;
  ...
  return (
    ...
    >
      {columns.map((col, i) => (
        <div key={i} className="table-header">{col}</div>
      ))}
      <button className="table-header table-delete" onClick={onAddRow}>+</button>
      {rows.map((row, r) => (
        <Fragment key={r}>
          {row.map((value, c) => (
            <input key={c}
          ...
          <button className="table-cell table-delete"
